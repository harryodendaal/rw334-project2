...
def csv_to_sql():
    ...
    with engine.connect() as con:
        ...
        con.execute('ALTER TABLE recipient_info ADD CONSTRAINT midfk FOREIGN KEY (mid) REFERENCES message (mid) MATCH FULL;')
        con.close()
######
...
def full_to_sender(fullname):
    ...
    cur = con.cursor()
    cur.execute("SELECT employee_list.email_id FROM employee_list WHERE lower(firstname) = \'" + fname + "\' OR lower(lastname) = \'" + sname + "\';")
    row = cur.fetchone()
    con.close()
    if row is None:
...
def most_contacted(email):
    ...
    most_contact = []
    loop = 5
    if len(set(all_contact)) <= 5: loop = len(set(all_contact))
    for i in range(loop):
        highest = most_common(all_contact)
        most_contact.append(highest)
        all_contact = list(filter(lambda a: a != highest, all_contact)) #removes most occuring
...
def num_mail(email, startdate, enddate):
    ...
    con = engine.raw_connection()
    cur = con.cursor()
    cur.execute("SELECT message.mid FROM message WHERE sender = \'" + email +"\' AND date BETWEEN \'" + str(startdate) +"\' AND \'" + str(enddate)+"\';")
    row = cur.fetchall()
    cur.close()
    con.close()
    return len(row)
...
def what_to_do():
    ...
    name = "allen" #get name from html form (Only name,/Only surname/Name Surname)
    startdate, enddate = "2001/01/01", "2001/05/27" #get dates (maybe use datepickers)
    ...
    i = num_mail(email, startdate, enddate) #number of mails SENT between dates given
    print(most)
    print(i)
